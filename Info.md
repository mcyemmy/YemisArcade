...
- Angular 4 (unlike AngularJS1) is an IDF, Integrated Development Framework where you have all the parts and pieces required for building a fully functional web application. You have the Component, Services, Directives and Router.
...
- An entire Angular 4 project are bootstrapped through main.ts to run in the browser. main.ts [points to] > appModule [which points to] > root Component class in app.component.ts [this through selector, templateUrl, styleUrls annotations points to] > <approot> element in index.html, custom markups in app.component.html and appp.component.css respectively
...
- Component - A component is an encapsulation of template, data and typescript class that are exposed to the Angular application through metadata. In otherwords, Components contain the HTML template view, behavior logic and data model. Every angular application contains at least one component called the root component. Components sometimes contain multiple smaller more manageable components.
- Directives - These are used to modify DOM elements and/or extend their behaviors. It extends HTML INTO the web application that it really should be. Directives can be structural directives to alter the DOM or can be attribute directives to alter appearance and behavior. While some are native to Angular, custom directives can also be created.
...
- Data binding - Synchronize data between Model and View - two-way binding, property binding, event binding 
