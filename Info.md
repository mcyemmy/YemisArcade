...
- [Promises]- Asynchronous event is a process which allows me to go take care of the rest of my day activities while I delegate a **CALLBACK** function(myBuddy) to handle sub-task for any of my activities which has one or more dependencies. These callback functions(myBuddies) will give me a shout out to come back and wrap up the dependency-ridden activities once all its dependencies have been resolved. A promise does about the same thing as a callback, just in a little different way (Kind of more readable and has error catching ability). So with a **PROMISE**, I delegate my activies that has dependent subtask to myBuddy while I go take care of the other day's activities. I make my buddy create a "new promise" for this activty which is followed by another subtask(.then) and after all is resolved I either get a resolved(done) or reject(error) shout out. So the promise task itself is synchronous in nature because it has dependencies that has to be completed before that one activity can be marked as complete. But my other activities that I was able to work on while the promised activity was ongoing gives me that **ASYNCHRONOUS** feature. Similarities => Callback functions are useful for single threaded asynchronous operations that nest those functions within one another as parameters. You pass the result of one operation as a argument. Promises lets you improve on such operation by writing your code as if they are sychronous operation. Differences => Promises provides more manageable code(using keywords like resolve, reject, then and catch) over callbacks and promises makes it smooth and easy to catch errors at every step when one of the promises are rejected and can perform a separate operation based on the caught error. [Practical tips on Callbacks](https://medium.freecodecamp.org/model-view-controller-mvc-explained-through-ordering-drinks-at-the-bar-efcba6255053), [More on Callbacks](https://medium.freecodecamp.org/understanding-asynchronous-javascript-callbacks-through-household-chores-e3de9a1dbd04), [Promises](https://scotch.io/tutorials/javascript-promises-for-dummies)
